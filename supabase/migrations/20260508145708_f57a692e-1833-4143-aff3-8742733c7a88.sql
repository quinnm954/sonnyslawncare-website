CREATE OR REPLACE FUNCTION public.submit_estimate_decision(_token text, _line_items jsonb, _status text, _signature text, _decline_reason text, _requested_date date DEFAULT NULL::date, _requested_time_window text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Cascade cleanup whenever a previously approved estimate is now declined
  -- or downgraded to partially_approved (some items declined).
  v_should_cascade := v_was_approved
    AND v_est.appointment_id IS NOT NULL
    AND (
      _status = 'declined'
      OR (_status = 'partially_approved' AND v_est.status = 'approved')
      OR (_status = 'partially_approved' AND v_est.status = 'partially_approved')
    );
